article_lab_css <- function() {
  v2_css <- if (get0("article_lab_design_v2", inherits = TRUE, ifnotfound = FALSE)) {
    c(
      ...
      "      .app-shell-v2 .guide { padding: 20px 18px; }",
      "      .app-shell-v2 .main {",
      "        max-width: 1060px;",
      "        margin: 0 auto;",
  ...
  HTML(paste(c(
    ...
    "        background: #ffffff;",
    "        box-shadow: 0 -10px 28px rgba(20, 31, 43, .16);",
    "        display: flex;",
...
article_lab_js <- function(is_dimension_mode_value = get0("is_dimension_mode", inherits = TRUE, ifnotfound = FALSE)) {
  HTML(paste0(
      ...
      "        const v2WideSections = ['research_inbox', 'summary', 'generate', 'api_scoring', 'subtitle_generation', 'thumbnails', 'outline', 'full_text', 'review_publish', 'settings'];",
      ...
      "        const evidenceDrawer = document.querySelector('.research-evidence-drawer');",
      "        const evidenceMarker = event.target && event.target.closest ? event.target.closest('.research-citation-marker') : null;",
      "        const insideEvidenceDrawer = event.target && event.target.closest ? event.target.closest('.research-evidence-drawer') : null;",
